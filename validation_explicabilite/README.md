...
2. MetaQuery `--execute` (repo `metaquery_actuary`), puis copier `extract.csv` ici et `manifest.json` dans `outputs/` — extraction gouvernée + manifeste de traçabilité ✅
3. **`pipeline_xai.py` — : entraînement+AUC → SHAP (beeswarm + 2
...
4. Lire `outputs/rapport_validation_explicabilite.md`

...
def build_report(imp_shap, lime_top, auc_train, auc_test, idx_A, idx_B, X_test, model,
    ...
    L(f"- Source : `{man['source']}` — statut requête : **{man['statut_requete']}**")
    L(f"- Extraction : `{man['extraction']['fichier']}` — {man['extraction']['lignes']} lignes — SHA-256 `{man['extraction']['sha256'][:16]}…`")
    L(f"- Qualité : **{man['qualite']['verdict']}** — contrôles : {', '.join(man['qualite']['controles'].keys())}")
